#!/usr/bin/env python3
"""
Sorter Worker | UI for manually tagging questions with topics.

Key fixes vs previous version
===============================
• Image paths are resolved relative to config.RAW_QUESTIONS_DIR so stale
  absolute paths from old DB entries never break the UI.
• When a topic is assigned the QP image AND its matching MS image (if any)
  are both copied into  sorted_questions/<topic>/<question_id>/  so every
  question folder contains both qp_<id>.png and ms_<id>.png side by side.
• Subject selector defaults to the first subject in config — never hardcoded.
• Topic panel is scrollable so many topics never cover the question image.
• Canvas mouse-wheel scrolling works on Linux and Windows/Mac.
"""
...
def _resolve_image_path(stored_path: str) -> str:
    ...
    working directory.  We always prefer a file that actually exists; if the
    stored path is broken we fall back to looking for the same filename in
    config.RAW_QUESTIONS_DIR.
    """
    if stored_path and os.path.exists(stored_path):
        return stored_path
    if stored_path:
        basename = os.path.basename(stored_path)
        candidate = os.path.join(config.RAW_QUESTIONS_DIR, basename)
        if os.path.exists(candidate):
            return candidate
    return stored_path  # caller will handle the error so we can js return ts as-is
...
class QuestionSorterUI:
    def __init__(self, root: tk.Tk):
        ...
        first_subject = next(iter(config.SUBJECTS))
    ...
    def _load_questions(self):
        ...
        if not os.path.exists(self.db_path):
            logger.warning("No question database found")
            return

        with open(self.db_path) as f:
            db = json.load(f)

        # Build set of IDs already in queue to avoid duplicates on live reload
        existing_ids = {q["id"] for q in self.question_queue}

        added = 0
        for pdf_name, pdf_data in db.items():
            if "_qp_" not in pdf_name:
                continue   # skip MS entries 
        ...
        if added:
            logger.info("Loaded %d untagged questions (total: %d)", added, len(self.question_queue))
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="TopicalForge  |  Question Sorter",
                 font=FONT_TITLE, bg=SURFACE, fg=TEXT).pack(side=tk.LEFT, padx=20, pady=14)

        self.progress_label = tk.Label(header, text="", font=FONT_BODY, bg=SURFACE, fg=TEXT_DIM)
        self.progress_label.pack(side=tk.RIGHT, padx=20)

        # MS badge — shows whether the current question has a mark scheme
        ...
        # Meta bar
        self.meta_label = tk.Label(self.root, text="", font=FONT_MONO,
                                   bg=BG, fg=TEXT_DIM, anchor="w", padx=20, pady=6)
        ...
        tk.Label(subj_frame, text="SUBJECT", font=(FONT_BODY[0], 9, "bold"),
                 bg=SURFACE, fg=TEXT_DIM).pack(anchor="w")

        for code, info in config.SUBJECTS.items():
            tk.Radiobutton(
                subj_frame, text=info["label"],
        ...
        tk.Label(panel, text="ASSIGN TOPIC", font=(FONT_BODY[0], 9, "bold"),
                 bg=SURFACE, fg=TEXT_DIM, padx=12).pack(anchor="w")
        ...
        topics_scroll       = tk.Scrollbar(topics_outer, orient=tk.VERTICAL,
                                           command=self._topics_canvas.yview)
        ...
        for text, cmd in [("← Prev", self._prev), ("Skip ->", self._skip)]:
            tk.Button(nav, text=text, command=cmd, font=FONT_BODY, bd=0,
                      bg=BORDER, fg=TEXT, activebackground=ACCENT,
                      activeforeground="white", padx=8, pady=5, cursor="hand2",
    ...
    def _rebuild_topic_buttons(self):
        ...
        topics = config.SUBJECTS.get(code, {}).get("topics", [])
        for topic in topics:
            btn = tk.Button(
                self._topics_inner, text=topic, command=lambda t=topic: self._assign(t),
                font=FONT_BODY, bd=0, bg=BG, fg=TEXT,
    ...
    def _load_question(self):
        ...
        subj_label = config.SUBJECTS.get(q["subject_code"], {}).get("label", q["subject_code"])
        has_ms     = bool(q.get("ms_image_path"))
        self.ms_badge.config(text="MS" if has_ms else "")
        self.meta_label.config(
            text=f"{subj_label}   ·   {q['session'].upper()}   ·   "
                 f"Paper {q['paper_num']}   ·   Q{q['question_num']}"
        )

        if q["subject_code"] in config.SUBJECTS:
        ...
        try:
            ...
            if img.width > canvas_w:
                ...
                img   = img.resize((canvas_w, int(img.height * ratio)), Image.Resampling.LANCZOS)
            ...
            self.meta_label.config(text=f"⚠  Image not found: {os.path.basename(img_path or '')}")

    def _show_done(self):
        self.progress_label.config(text="All done")
        self.meta_label.config(text="All questions tagged — you can close this window.")
        self.canvas.delete("all")
        self.canvas.create_text(400, 200, text="All questions sorted!",
                                fill=SUCCESS, font=(FONT_TITLE[0], 20, "bold"))
    ...
    def _bundle_to_sorted(self, q: dict, topic: str):
        """
        Copy QP image (and MS image if available) into:
    ...
    def _quick_assign(self, idx: int):
        topics = config.SUBJECTS.get(self.subject_var.get(), {}).get("topics", [])
        if idx < len(topics):
...
class QuestionWatcher(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory and event.src_path.endswith(".png"):
            logger.info("New image: %s", os.path.basename(event.src_path))
            self.ui._load_questions()
...
def run_sorter():
    ...
    root = tk.Tk()
    app  = QuestionSorterUI(root)

    handler  = QuestionWatcher(app)
    observer = Observer()
